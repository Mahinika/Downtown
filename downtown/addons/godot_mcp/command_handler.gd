...
func handle_command(command_type, params):
	match command_type:
		...
		"CONNECT_SIGNAL":
			return handle_connect_signal(params)
		"DISCONNECT_SIGNAL":
			return handle_disconnect_signal(params)
		"LIST_SIGNAL_CONNECTIONS":
			return handle_list_signal_connections(params)
		"GET_SIGNAL_LIST":
			return handle_get_signal_list(params)
		# Node groups commands
		"ADD_NODE_TO_GROUP":
			return handle_add_node_to_group(params)
		"REMOVE_NODE_FROM_GROUP":
			return handle_remove_node_from_group(params)
		"GET_NODES_IN_GROUP":
			return handle_get_nodes_in_group(params)
		"LIST_ALL_GROUPS":
			return handle_list_all_groups()
		# Autoload management commands
		"GET_AUTOLOADS":
			return handle_get_autoloads()
		"SET_AUTOLOAD":
			return handle_set_autoload(params)
		# Project settings commands
		"GET_PROJECT_SETTING":
			return handle_get_project_setting(params)
		"SET_PROJECT_SETTING":
			return handle_set_project_setting(params)
		# TileMap operations commands
		"PAINT_TILE":
			return handle_paint_tile(params)
		"ERASE_TILE":
			return handle_erase_tile(params)
		"GET_TILE_INFO":
			return handle_get_tile_info(params)
		"CREATE_TILESET":
			return handle_create_tileset(params)
		# Export variables commands
		"GET_EXPORT_VARIABLES":
			return handle_get_export_variables(params)
		"SET_EXPORT_VARIABLE":
			return handle_set_export_variable(params)
		# Bulk operations commands
		"BULK_CREATE_NODES":
			return handle_bulk_create_nodes(params)
		"BULK_SET_PROPERTY":
			return handle_bulk_set_property(params)
		"DUPLICATE_SUBTREE":
			return handle_duplicate_subtree(params)
		# UI layout helper commands
		"SET_ANCHOR":
			return handle_set_anchor(params)
		"SET_MARGIN":
			return handle_set_margin(params)
		"APPLY_THEME":
			return handle_apply_theme(params)
		"LIST_TOOLS":
...
func handle_create_child_object(params):
	...
	# Check if name already exists and handle replacement
	if node_name != "":
		var existing_node = current_scene.find_child(node_name, true, false)
		if existing_node and not replace_if_exists:
			return {"error": "Node with name '" + node_name + "' already exists. Use replace_if_exists=true to replace it."}
		elif existing_node and replace_if_exists:
			existing_node.queue_free()
...
# ==============================================================================
# PHASE 1: Signal System, Node Groups, and Autoloads
# ==============================================================================

# Signal system commands
func handle_connect_signal(params):
	if not params.has("node_from") or not params.has("signal_name") or not params.has("node_to") or not params.has("method_name"):
		return {"error": "Missing required parameters: node_from, signal_name, node_to, method_name"}
	
	var node_from_name = params.node_from
	var signal_name = params.signal_name
	var node_to_name = params.node_to
	...
	# Find source node
	var node_from = _find_node_by_name_or_path(current_scene, node_from_name)
	if not node_from:
		return {"error": "Source node not found: " + node_from_name}
	
	# Find target node
	var node_to = _find_node_by_name_or_path(current_scene, node_to_name)
	if not node_to:
		return {"error": "Target node not found: " + node_to_name}
	
	# Check if signal exists
	if not node_from.has_signal(signal_name):
		return {"error": "Signal '" + signal_name + "' does not exist on node '" + node_from_name + "'"}
	
	# Connect the signal
	# Create Callable - binds not yet supported in this implementation
	var callable = Callable(node_to, method_name)
	# Note: binds parameter is accepted but not yet implemented
	# TODO: Implement binds support when needed
	var error = node_from.connect(signal_name, callable, flags)
	if error != OK:
		return {"error": "Failed to connect signal: Error code " + str(error)}
	
	return {
		"success": true,
		"message": "Signal '" + signal_name + "' connected from '" + node_from_name + "' to '" + node_to_name + "." + method_name + "'"
	}

func handle_disconnect_signal(params):
	if not params.has("node_from") or not params.has("signal_name"):
		return {"error": "Missing required parameters: node_from, signal_name"}
	
	var node_from_name = params.node_from
	var signal_name = params.signal_name
	var node_to_name = params.get("node_to", "")
	...
	if node_to_name != "" and method_name != "":
		# Disconnect specific connection
		var node_to = _find_node_by_name_or_path(current_scene, node_to_name)
		if not node_to:
			return {"error": "Target node not found: " + node_to_name}
		disconnected = node_from.is_connected(signal_name, Callable(node_to, method_name))
		if disconnected:
			node_from.disconnect(signal_name, Callable(node_to, method_name))
	else:
		# Disconnect all connections for this signal
		if node_from.has_signal(signal_name):
			var connections = node_from.get_signal_connection_list(signal_name)
			for connection in connections:
				node_from.disconnect(signal_name, connection["callable"])
			disconnected = len(connections) > 0
	
	if not disconnected:
		return {"error": "No connections found to disconnect for signal '" + signal_name + "'"}
	
	return {
		"success": true,
		"message": "Signal '" + signal_name + "' disconnected from node '" + node_from_name + "'"
	}

func handle_list_signal_connections(params):
	if not params.has("node_name"):
	...
	# Find node
	var node = _find_node_by_name_or_path(current_scene, node_name)
	if not node:
		return {"error": "Node not found: " + node_name}
	...
	var all_connections = []
	if signal_name != "":
		# Get connections for specific signal
		if node.has_signal(signal_name):
			var connections = node.get_signal_connection_list(signal_name)
			for conn in connections:
				all_connections.append({
					"signal": signal_name,
					"target": conn["callable"].get_object().get_path() if conn["callable"].is_valid() else "",
					"method": conn["callable"].get_method() if conn["callable"].is_valid() else "",
...
func handle_get_signal_list(params):
	if not params.has("node_name"):
		return {"error": "Missing required parameter: node_name"}
	...
	for signal_info in signal_list:
		signals.append({
			...
			"return_value": signal_info.get("return_value", null)
		})
...
# Node groups commands
func handle_add_node_to_group(params):
	if not params.has("node_name") or not params.has("group_name"):
		return {"error": "Missing required parameters: node_name, group_name"}
	...
	return {
		"success": true,
		"message": "Node '" + node_name + "' added to group '" + group_name + "'"
	}

func handle_remove_node_from_group(params):
	if not params.has("node_name") or not params.has("group_name"):
		return {"error": "Missing required parameters: node_name, group_name"}
	...
	if not node.is_in_group(group_name):
		return {"error": "Node '" + node_name + "' is not in group '" + group_name + "'"}
	...
	return {
		"success": true,
		"message": "Node '" + node_name + "' removed from group '" + group_name + "'"
	}

func handle_get_nodes_in_group(params):
	if not params.has("group_name"):
	...
	# Get nodes in group
	var nodes_in_group = current_scene.get_tree().get_nodes_in_group(group_name)
	var node_list = []
...
func handle_list_all_groups():
	...
	# Get all unique groups in the scene
	var groups = {}
	_collect_groups_recursive(current_scene, groups)
	
	var group_list = []
	for group_name in groups:
		group_list.append({
			"name": group_name,
			"node_count": groups[group_name]
		})
...
# Autoload management commands
func handle_get_autoloads():
	var autoloads = []
	
	# Get autoload list from project settings
	var autoload_list = ProjectSettings.get_setting("autoload/list")
	if autoload_list != null:
		for i in range(autoload_list.size()):
			var autoload_name = autoload_list[i]
			var autoload_path = ProjectSettings.get_setting("autoload/" + autoload_name)
			var autoload_singleton = ProjectSettings.get_setting("autoload/" + autoload_name + "/singleton")
			var autoload_enabled = ProjectSettings.get_setting("autoload/" + autoload_name + "/enabled")
			
			autoloads.append({
				"name": autoload_name,
				"path": autoload_path if autoload_path != null else "",
				"singleton": autoload_singleton if autoload_singleton != null else false,
				"enabled": autoload_enabled if autoload_enabled != null else true
			})
...
func handle_set_autoload(params):
	if not params.has("singleton_name"):
		return {"error": "Missing required parameter: singleton_name"}
	
	var singleton_name = params.singleton_name
	var path = params.get("path", "")
	var enabled = params.get("enabled", true)
	
	# For autoload modification, we need to edit project.godot directly
	# This is more complex as it requires file parsing/editing
	# For now, return a message indicating this needs project.godot file editing
	return {
		"success": false,
		"message": "Autoload modification requires project.godot file editing. Use ProjectSettings API or edit project.godot manually.",
		"note": "Autoloads are configured in project.godot under [autoload] section"
	}

# Helper function to find node by name or path
func _find_node_by_name_or_path(root: Node, name_or_path: String) -> Node:
	if "/" in name_or_path:
		# It's a path
		var node = root.get_node_or_null(name_or_path)
		if not node and not name_or_path.begins_with("/"):
		...
		# It's a name, search recursively
		return root.find_child(name_or_path, true, false)

# Helper function to collect all groups recursively
func _collect_groups_recursive(node: Node, groups: Dictionary):
	if node == null:
		return
...
# ==============================================================================
# PHASE 2: Project Settings, TileMap Operations, Export Variables
# ==============================================================================

# Project settings commands
func handle_get_project_setting(params):
	if not params.has("setting_name"):
		return {"error": "Missing required parameter: setting_name"}
	
	var setting_name = params.setting_name
	...
	if ProjectSettings.has_setting(setting_name):
		...
		return {"error": "Project setting not found: " + setting_name}

func handle_set_project_setting(params):
	if not params.has("setting_name") or not params.has("value"):
		return {"error": "Missing required parameters: setting_name, value"}
	...
	return {
		"success": true,
		"message": "Project setting '" + setting_name + "' set successfully"
	}

# TileMap operations commands
func handle_paint_tile(params):
	if not params.has("tilemap_node"):
		return {"error": "Missing required parameter: tilemap_node"}
	
	var tilemap_node_name = params.tilemap_node
	var cell_pos = params.get("cell_pos", [0, 0])
	var source_id = params.get("source_id", 0)
	var atlas_coords = params.get("atlas_coords", [0, 0])
	var alternative_id = params.get("alternative_id", 0)
	...
	# Find TileMap node
	var tilemap = _find_node_by_name_or_path(current_scene, tilemap_node_name)
	if not tilemap:
		return {"error": "TileMap node not found: " + tilemap_node_name}
	
	if not tilemap is TileMap:
		return {"error": "Node is not a TileMap: " + tilemap_node_name}
	...
	# Paint tile
	var coord = Vector2i(cell_pos[0], cell_pos[1])
	var atlas = Vector2i(atlas_coords[0], atlas_coords[1])
	layer.set_cell(coord, source_id, atlas, alternative_id)
	
	return {
		"success": true,
		"message": "Tile painted at (" + str(cell_pos[0]) + ", " + str(cell_pos[1]) + ")"
	}

func handle_erase_tile(params):
	if not params.has("tilemap_node"):
		return {"error": "Missing required parameter: tilemap_node"}
	...
	return {
		"success": true,
		"message": "Tile erased at (" + str(cell_pos[0]) + ", " + str(cell_pos[1]) + ")"
	}

func handle_get_tile_info(params):
	if not params.has("tilemap_node"):
		return {"error": "Missing required parameter: tilemap_node"}
	...
	var atlas_coords = layer.get_cell_atlas_coords(coord)
	var alternative_id = layer.get_cell_alternative_tile(coord)
...
func handle_create_tileset(params):
	if not params.has("atlas_path"):
		return {"error": "Missing required parameter: atlas_path"}
	
	var atlas_path = params.atlas_path
	
	# Check if atlas exists
	if not FileAccess.file_exists(atlas_path):
		return {"error": "Atlas texture not found: " + atlas_path}
	...
	# Save tileset (would need a save path)
	return {
		"success": true,
		"message": "TileSet created with atlas: " + atlas_path,
		"note": "TileSet resource created in memory. Use SET_PROPERTY to assign it to a TileMap."
	}

# Export variables commands
func handle_get_export_variables(params):
	if not params.has("node_name"):
		return {"error": "Missing required parameter: node_name"}
	...
	# Get export variables from script
	var export_vars = []
	if node.get_script():
		var script = node.get_script()
		var property_list = node.get_property_list()
		for prop in property_list:
			if prop.get("usage", 0) & PROPERTY_USAGE_SCRIPT_VARIABLE and prop.get("usage", 0) & PROPERTY_USAGE_STORAGE:
				if prop.get("name", "").begins_with("_export_"):
					var var_name = prop["name"].substr(8)  # Remove "_export_" prefix
					export_vars.append({
						"name": var_name,
						"type": prop.get("type", TYPE_NIL),
						"value": node.get(var_name) if var_name in node else null
					})
...
func handle_set_export_variable(params):
	if not params.has("node_name") or not params.has("variable_name") or not params.has("value"):
		return {"error": "Missing required parameters: node_name, variable_name, value"}
	...
	# Set export variable (export vars are regular properties)
	if variable_name in node:
		node.set(variable_name, value)
		return {
			"success": true,
			"message": "Export variable '" + variable_name + "' set on node '" + node_name + "'"
		}
	else:
		return {"error": "Variable '" + variable_name + "' not found on node '" + node_name + "'"}

# ==============================================================================
# PHASE 3: Bulk Operations and UI Layout Helpers
# ==============================================================================

# Bulk operations commands
func handle_bulk_create_nodes(params):
	if not params.has("parent_name"):
		return {"error": "Missing required parameter: parent_name"}
	
	var parent_name = params.parent_name
	var node_specs = params.get("node_specs", [])
	
	if node_specs.size() == 0:
		return {"error": "node_specs array is empty"}
	...
	# Find parent node
	var parent = _find_node_by_name_or_path(current_scene, parent_name)
	if not parent:
		return {"error": "Parent node not found: " + parent_name}
	...
	for spec in node_specs:
		var node_type = spec.get("type", "EMPTY")
		var node_name = spec.get("name", "")
		var location = spec.get("location", [0, 0, 0])
		var rotation = spec.get("rotation", [0, 0, 0])
		var scale = spec.get("scale", [1, 1, 1])
		
		# Create node (reuse logic from handle_create_object)
		var node = null
...
func handle_bulk_set_property(params):
	if not params.has("node_list") or not params.has("property_name") or not params.has("value"):
		return {"error": "Missing required parameters: node_list, property_name, value"}
	...
	if node_list.size() == 0:
		return {"error": "node_list array is empty"}
	...
	return {
		...
		"total_nodes": node_list.size()
	}

func handle_duplicate_subtree(params):
	if not params.has("node_name"):
		return {"error": "Missing required parameter: node_name"}
	
	var node_name = params.node_name
	var new_parent_name = params.get("new_parent_name", "")
	...
	# Find source node
	var source_node = _find_node_by_name_or_path(current_scene, node_name)
	if not source_node:
		return {"error": "Source node not found: " + node_name}
	
	# Find new parent (default to scene root)
	var new_parent = current_scene
	if new_parent_name != "":
		new_parent = _find_node_by_name_or_path(current_scene, new_parent_name)
		if not new_parent:
			return {"error": "New parent node not found: " + new_parent_name}
	
	# Duplicate node and subtree
	var duplicate = source_node.duplicate(Node.DUPLICATE_SCRIPTS | Node.DUPLICATE_GROUPS | Node.DUPLICATE_SIGNALS)
	if not duplicate:
	...
	# Add to new parent
	new_parent.add_child(duplicate)
	duplicate.owner = current_scene
	_set_owner_recursive(duplicate, current_scene)
	
	return {
		"success": true,
		"duplicate_name": duplicate.name,
		"duplicate_path": current_scene.get_path_to(duplicate)
	}

# UI layout helper commands
func handle_set_anchor(params):
	if not params.has("control_node") or not params.has("anchor_side") or not params.has("anchor_value"):
		return {"error": "Missing required parameters: control_node, anchor_side, anchor_value"}
	
	var control_node_name = params.control_node
	var anchor_side = params.anchor_side  # "left", "top", "right", "bottom"
	var anchor_value = params.anchor_value
	...
	# Find control node
	var control = _find_node_by_name_or_path(current_scene, control_node_name)
	if not control:
		return {"error": "Control node not found: " + control_node_name}
	
	if not control is Control:
		return {"error": "Node is not a Control: " + control_node_name}
	
	# Set anchor based on side
	match anchor_side.to_lower():
		"left":
			control.set_anchor(SIDE_LEFT, anchor_value, false, true)
		"top":
			control.set_anchor(SIDE_TOP, anchor_value, false, true)
		"right":
			control.set_anchor(SIDE_RIGHT, anchor_value, false, true)
		"bottom":
			control.set_anchor(SIDE_BOTTOM, anchor_value, false, true)
		_:
			return {"error": "Invalid anchor_side. Must be: left, top, right, or bottom"}
	
	return {
		"success": true,
		"message": "Anchor '" + anchor_side + "' set to " + str(anchor_value) + " on node '" + control_node_name + "'"
	}

func handle_set_margin(params):
	if not params.has("control_node") or not params.has("side") or not params.has("margin_value"):
		return {"error": "Missing required parameters: control_node, side, margin_value"}
	
	var control_node_name = params.control_node
	var side = params.side  # "left", "top", "right", "bottom"
	var margin_value = params.margin_value
	...
	# Set margin based on side
	match side.to_lower():
		"left":
			control.offset_left = margin_value
		"top":
			control.offset_top = margin_value
		"right":
			control.offset_right = margin_value
		"bottom":
			control.offset_bottom = margin_value
		_:
			return {"error": "Invalid side. Must be: left, top, right, or bottom"}
	
	return {
		"success": true,
		"message": "Margin '" + side + "' set to " + str(margin_value) + " on node '" + control_node_name + "'"
	}

func handle_apply_theme(params):
	if not params.has("control_node"):
		return {"error": "Missing required parameter: control_node"}
	...
	# Load and apply theme if path provided
	if theme_path != "":
		if not FileAccess.file_exists(theme_path):
			return {"error": "Theme file not found: " + theme_path}
		
		var theme = load(theme_path)
		if not theme is Theme:
			return {"error": "File is not a Theme resource: " + theme_path}
		
		control.theme = theme
		return {
			"success": true,
			"message": "Theme applied to node '" + control_node_name + "'"
		}
	else:
		# Clear theme
		control.theme = null
		return {
			"success": true,
			"message": "Theme cleared from node '" + control_node_name + "'"
		}

# Tool discovery command
func handle_list_tools():
	# Return list of all available tools with their schemas
	# This allows MCP bridges to dynamically discover available tools
	var tools = []
	...
	tools.append({
		"name": "mcp_godot_open_scene",
		"command_type": "OPEN_SCENE",
		"description": "Open a scene from the project",
		"parameters": {
			"type": "object",
			"properties": {
				"scene_path": {"type": "string", "description": "Path to the scene file"},
				"save_current": {"type": "boolean", "description": "Whether to save the current scene first"}
			}
		}
	})
	tools.append({
		"name": "mcp_godot_save_scene",
		"command_type": "SAVE_SCENE",
		"description": "Save the current scene",
		"parameters": {"type": "object", "properties": {}}
	})
	tools.append({
		"name": "mcp_godot_new_scene",
		"command_type": "NEW_SCENE",
		"description": "Create a new empty scene",
		"parameters": {
			"type": "object",
			"properties": {
				"scene_path": {"type": "string", "description": "Path where the new scene should be saved"},
				"overwrite": {"type": "boolean", "description": "Whether to overwrite if the scene already exists"}
			}
		}
	})
	tools.append({
		"name": "mcp_godot_get_hierarchy",
		"command_type": "GET_SCENE_INFO",
		"description": "Get the detailed hierarchy of objects in the current scene",
		"parameters": {"type": "object", "properties": {}}
	})
	
	# Node operations tools (add key ones)
	tools.append({
		"name": "mcp_godot_create_object",
		"command_type": "CREATE_OBJECT",
		"description": "Create a new object (node) in the current scene",
		"parameters": {
			"type": "object",
			"properties": {
				"type": {"type": "string", "description": "Type of object to create"},
				"name": {"type": "string", "description": "Optional name for the new object"},
				"location": {"type": "array", "description": "Optional [x, y, z] position"},
				"rotation": {"type": "array", "description": "Optional [x, y, z] rotation in degrees"},
				"scale": {"type": "array", "description": "Optional [x, y, z] scale factors"}
			}
		}
	})
	
	# Signal system tools (NEW)
	tools.append({
		"name": "mcp_godot_connect_signal",
		"command_type": "CONNECT_SIGNAL",
	...
	tools.append({
		...
		"command_type": "DISCONNECT_SIGNAL",
	...
	tools.append({
		...
		"command_type": "LIST_SIGNAL_CONNECTIONS",
	...
	# Node groups tools (NEW)
	tools.append({
		"name": "mcp_godot_add_node_to_group",
		"command_type": "ADD_NODE_TO_GROUP",
	...
	tools.append({
		...
		"command_type": "REMOVE_NODE_FROM_GROUP",
	...
	tools.append({
		...
		"command_type": "GET_NODES_IN_GROUP",
	...
	# Project settings tools (NEW)
	tools.append({
		"name": "mcp_godot_get_project_setting",
		"command_type": "GET_PROJECT_SETTING",
	...
	tools.append({
		...
		"command_type": "SET_PROJECT_SETTING",
	...
	# Export variables tools (NEW)
	tools.append({
		"name": "mcp_godot_get_export_variables",
		"command_type": "GET_EXPORT_VARIABLES",
	...
	tools.append({
		...
		"command_type": "SET_EXPORT_VARIABLE",
	...
	# Bulk operations tools (NEW)
	tools.append({
		"name": "mcp_godot_bulk_create_nodes",
		"command_type": "BULK_CREATE_NODES",
	...
	tools.append({
		...
		"command_type": "BULK_SET_PROPERTY",
		...
		"parameters": {
			...
			"properties": {
				"operations": {
					"type": "array",
					"description": "Array of property update operations"
				}
			}
		}
	})
	tools.append({
		...
		"command_type": "DUPLICATE_SUBTREE",
